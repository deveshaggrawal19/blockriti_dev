...
angular.module('taurus.dashboardModule')
    .controller('DashboardCtrl', ["$scope", '$rootScope', "authenticationService", "dashboardService", "$auth", "$cookies", "$state", "urlService", "$mdMedia", "$timeout", "$mdDialog", "toastMessagesService", "$interval", "$firebaseArray", "$firebaseObject", "$mdExpansionPanel", "$http",
      function($scope, $rootScope, authenticationService, dashboardService, $auth, $cookies, $state, urlService, $mdMedia, $timeout, $mdDialog, toastMessagesService, $interval, $firebaseArray, $firebaseObject, $mdExpansionPanel, $http) {
     ...
     vm.allowedCountries = [ 
      ...
      'Zimbabwe' 
	   ...
	   function getUserInfo() {
            var data = {"test": "true"};
            dashboardService.getUserInfo(data, function successBlock(data) {
                $scope.userinfo = data;
                ...
                var data = {"test": "true"};
                dashboardService.getReferral(data, function successBlock(data) {
    ...
    var verificationFailureToast = function(code) {
      switch (code) {
       ...
       default:
        ...
        var data = {'test': 'true'};
        dashboardService.getVerification(data,function successBlock(data) {
    ...
    function getLevelOneDetails() {
      dashboardService.getLevelOneDetails(function successBlock(data) {
        console.log(data);
        vm.verificationOne = {
...
/************************************* Upload + Submit ********************************/
  ...
  var uploadSingleFile = function (formId, inputId, verificationTwo) {
   ...
   function submitVerificationOne(verificationOneData) {
      ...
      if (vm.verificationOneForm.$valid && vm.photoForm.$valid && vm.billUtilityForm.$valid) {
        ...
        dashboardService.verification_first(verificationOneSubmitData, function successBlock(data) {
          console.log('success');
  ...
  function submitVerificationTwo(verificationTwoData) {
    ...
    dashboardService.getVerification(dataToSend, function successBlock(data) {
      // vm.getVerificationData = data;
      console.log(data);
...
/*
    dashboardService.getVerification
